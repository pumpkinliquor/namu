			...
			<tbody>
				<tr>
					...
					<td colspan="2" class="t_tit">
						<span>[<c:out value="${custom1Name}"/>][<c:out value="${result.custom2}"/>]</span>
						<c:choose>
					...
					<td class="t_date t_right">
						<span><c:out value="${result.customDate1}"/> ~ <c:out value="${result.customDate2}"/></span>
						<c:choose>
							<c:when test="${diffDate gt 0}">
								<em class="c_gray3">진행중 <strong class="c_point fw_500">D-<c:out value="${diffDate }" /></span></em>
							</c:when>
							<c:when test="${diffDate eq 0}">
								<em class="c_gray3">진행중 <strong class="c_point fw_500">D-Day</span></em>
							</c:when>
							<c:otherwise>
								<em class="c_gray3">종료 </em>
							</c:otherwise>
						</c:choose>
					</td>
				</tr>
						...
						<c:choose>
							<c:when test="${diffDate ge 0}">
								<div class="btn_Lbox"><!-- btn box -->
									<a href="#none" onclick="javascript:fnJPARegist('${result.nttId}');" class="btn breservation w215">입사지원</a>
								</div>
							</c:when>
						</c:choose>
					</td>
