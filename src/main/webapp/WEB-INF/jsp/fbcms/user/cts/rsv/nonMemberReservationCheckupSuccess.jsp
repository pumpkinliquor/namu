...
<!-- contents s -->
		<div class="contents w1200">
			...
			<ul class="step reser_step"><!-- 진행 스텝 -->
				...
				<li class="end"><span>완료</span>검진정보 입력</li>
			...
			<section class="re_login">
				...
				<p>귀하의 비에비스 나무병원 검진 예약 신청이 완료되었습니다<br>
					예약하신 정보는 다음과 같습니다</p>
				<ul class="box_complete">
					<li>이름<strong>${userCheckupVO.usrNm}</strong></li>
					<li>휴대폰<strong>${userCheckupVO.fCtn}-${userCheckupVO.mCtn}-${userCheckupVO.lCtn}</strong></li>
					<li>검진프로그램<strong>${userCheckupVO.DTIL_NM}</strong></li>
					<%--
					<li>예약일시<strong>${fn:substring(userCheckupVO.RSVDT, 0,4)}.${fn:substring(userCheckupVO.RSVDT, 4,6)}.${fn:substring(userCheckupVO.RSVDT, 6,8)}<span>${fn:substring(userCheckupVO.RSVTM, 0,2)}:${fn:substring(userCheckupVO.RSVTM, 2,4)}</span></strong></li>
					 --%>
					<li>예약일시<strong>${fn:substring(userCheckupVO.RSVDT, 0,4)}.${fn:substring(userCheckupVO.RSVDT, 4,6)}.${fn:substring(userCheckupVO.RSVDT, 6,8)}</strong></li>
				</ul>
				<div class="btn_Lbox"><!-- btn box -->
					<a href="/index.do" class="btn bgreen">메인</a>
					<a href="javascript:fnGetProcess('/user/cts/rsv/nonMemberReservationMain.do');" class="btn bblue">예약조회</a>
				</div>
			</section>
