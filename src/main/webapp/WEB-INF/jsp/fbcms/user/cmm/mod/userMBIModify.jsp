...
<div class="container" id="contents">
	...
	<form name="userInfoModify" action="" method="post">
			...
			<table>
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<input type="password" class="w510" title="비밀번호" maxlength="16"" id="PASSWORD" name="PASSWORD">
							<p class="t_message">※ 영문 대/소문자, 숫자, 특수문자 중 3가지 이상 8자, 최대 16자까지 가능</p>
							<p class="t_message02" id="message01">* 비밀번호는 영문 대/소문자, 숫자, 특수문자 중  3가지 이상 8자 최대 16자 까지 입력 가능합니다.</p>
						</td>
					</tr>
					<tr>
						<th scope="row"><em class="ootc">*</em>비밀번호 확인</th>
						<td>
							<input type="password" class="w510" title="비밀번호 확인" maxlength="16" id="PASSWORDChk" name="PASSWORDChk">
							<p class="t_message02" id="message02">* 비밀번호가 일치하지 않습니다.</p>
						</td>
					</tr>
					<tr>
						<th scope="row"><em class="ootc_none"></em>계정 연동</th>
						<td>
							<p class="peristalsis"><span class="sns_naver">네이버</span><a href="javascript:void(0);" class="naverbtn btn"></a></p>
							<p class="peristalsis"><span class="sns_kakao">카카오</span><a href="javascript:void(0);" class="kakaobtn btn"></a></p>
							<p class="peristalsis"><span class="sns_google">구글</span><a href="javascript:void(0);" class="googlebtn btn"></a></p>

						</td>
					</tr>
					<tr>
						<th scope="row"><em class="ootc">*</em>휴대폰</th>
						<td><p class="peristalsis"><span id="mbt"><c:out value="${memberInfo.MBTLNUM }"/></span><a href="javascript:void(0);" class="btn bxs" id="authentication">번호변경</a></p>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
						<c:if test="${memberInfo.SEXDSTN_CODE eq 'F'}">
							<span>여자</span>
						</c:if>
						<c:if test="${memberInfo.SEXDSTN_CODE ne 'F'}">
							<span>남자</span>
						</c:if>
					...
					<tr>
						...
						<td>
							<input type="text" class="w245" title="이메일 입력" id="mailId">
							<span>@</span>
							<input type="text" class="w245" title="이메일 입력" id="domain">
					...
					<tr>
						...
						<td>
							<div>
								<input type="text" value="<c:out value='${memberInfo.ADRES }'/>" class="w510 disabled" title="주소입력" disabled>
								<a href="javascript:fnDaumPostcode()" class="btn bs ml5">주소등록</a>
							</div>
							<div>
								<input type="text" name="DETAIL_ADRES" id="DETAIL_ADRES" value="<c:out value='${memberInfo.DETAIL_ADRES}'/>" class="w510 disabled mt10" title="상세 주소입력">
							</div>
						</td>
					</tr>
					<tr>
						<th scope="row"><em class="ootc_none"></em>이메일 수신여부</th>
						<td>
							<c:if test="${memberInfo.EMAIL_RECV_YN eq 'Y'}">
							<input type="checkbox" name="EMAIL_RECV_YN" id="EMAIL_RECV_YN" checked>
							</c:if>
							<c:if test="${memberInfo.EMAIL_RECV_YN ne 'Y' or empty memberInfo.EMAIL_RECV_YN}">
		...
		<input type="hidden" name="googleToken" id="googleToken" value="${google.TOKEN_SN }"/>
		<input type="hidden" name="naverToken" id="naverToken" value="${naver.TOKEN_SN }"/>
		...
		<input type="hidden" name="ADRES" id="ADRES" value="${memberInfo.ADRES }"/>				
			...
			<div class="btn_Lbox"><!-- btn box -->
				<a href="javascript:fnCancel('${memberInfo.MBER_ID }')" class="btn bgreen">취소</a>
				<a href="javascript:fnModify()" class="btn bblue">수정</a>
				<a href="javascript:fnDrop()" class="btn bgray">회원탈퇴</a>
			</div>
		</section>
	</form>
...
$(function() {
	...
	if (EMAIL_RECV_YN === "Y") {
		...
		$("input:checkbox[name=EMAIL_RECV_YN]").prop("checked", true);
	} else {
		$("input:checkbox[name=EMAIL_RECV_YN]").prop("checked", false);
	}
	$("input:checkbox[name='EMAIL_RECV_YN']").on('click', function() {
		if ($("input:checkbox[name='EMAIL_RECV_YN']").is(":checked") == true) {
			$("input:checkbox[name=EMAIL_RECV_YN]").val('Y');
		} else {
			$("input:checkbox[name=EMAIL_RECV_YN]").val('N'); 
			}
		});
