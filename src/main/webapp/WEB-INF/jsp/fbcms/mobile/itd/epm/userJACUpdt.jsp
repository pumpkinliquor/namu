...
<div class="contents">
	...
	<form:form commandName="articleVO" name="formUpdate" action="/user/itd/epm/updateJAC.do" method="post" onSubmit="updateJAC(document.formUpdate); return false;" enctype="multipart/form-data">
			...
			<table>
				...
				<tbody>
					...
					<tr>
						...
						<td class="phone">
							<input type="text" id="custom4_1" name="custom4_1" title="이메일 입력" maxlength="100" value="${fn:split(articleVO.custom4, '@')[0]}" />
							<span>@</span>
							<input type="text" id="custom4_2" name="custom4_2" style="width:calc((100% - -8px) / 3)" title="이메일 입력" maxlength="100" value="${fn:split(articleVO.custom4, '@')[1]}" />
					...
					<tr>
						...
						<td>
							...
							<input type="text" id="b_file_txt" class="in_full" title="입사지원서 업로드" maxlength="40" placeholder="파일을 선택하세요" <c:if test="${not empty fileList and not empty fileList[0]}">value="${fileList[0].orignlFileNm }"</c:if> disabled="disabled" style="width:calc(100% - 108px)">
		...
		<div class="btn_Lbox">
			<a href="<c:url value='/user/itd/epm/employmentGuide.do'/>" class="btn btn02 bgreen">메인</a>
			<a href="#none" onclick="javascript:updateJAC(document.formUpdate)" class="btn btn02 bblue">수정</a>
		</div>
