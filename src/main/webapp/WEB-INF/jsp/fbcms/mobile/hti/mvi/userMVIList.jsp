...
<div class="contents">
	<form name="listForm" method="POST" action="<c:url value='/user/hti/mvi/selectMVIList.do'/>"><!-- 검색 -->
		<ul class="disease_box mar20">
			<li<c:if test="${searchVO.custom1 eq ''}"> class="active"</c:if>><a href="#none" onclick="javascipt:selectMVIList('')">전체</a>
			<c:forEach var="cmmCodeInfo" items="${listMVIComCode }" varStatus="status">
				<c:set var="customClass"></c:set>
				<c:if test="${searchVO.custom1 eq cmmCodeInfo.code}"><c:set var="customClass">active</c:set></c:if>
				<c:if test="${status.count + 1 eq fn:length(listMVIComCode) and (fn:length(listMVIComCode) + 1) % 3 eq 2}"><c:set var="customClass">${customClass} bott</c:set></c:if>
				<c:if test="${status.last and (fn:length(listMVIComCode) + 1) % 3 eq 2}"><c:set var="customClass">${customClass} bott right</c:set></c:if>
				<c:if test="${status.last and (fn:length(listMVIComCode) + 1) % 3 eq 1}"><c:set var="customClass">${customClass} bott right</c:set></c:if>
				<li class="${customClass}">
					<a href="#none" onclick="javascipt:selectMVIList('${cmmCodeInfo.code}')"><c:out value="${cmmCodeInfo.codeNm }" /></a>
				</li>
			</c:forEach>
		</ul>
	...
	<ul class="tbox">
		<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
			<li>
				<a href="#none" onclick="javascript:selectMVIDetail('<c:out value="${resultInfo.nttId}"/>')">
