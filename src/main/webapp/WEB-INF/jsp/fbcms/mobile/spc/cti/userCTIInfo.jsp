...
<div class="contents">
	<div class="tab swiper-container">
		<ul class="tab_list swiper-wrapper"><!-- tab -->
			<li class="selected swiper-slide"><a href="#none" onclick="funcOri()" title="선택된 탭">센터소개</a></li>
			<c:forEach var="clinicInfo" items="${listClinic }">
				<li class="swiper-slide"><a href="#none" onclick="funcMenu(${clinicInfo.mngSeq})"><c:out value="${clinicInfo.clinicNm }" /></a></li>
			</c:forEach>
		</ul>
	</div>
	...
	<c:if test="${result18.useAt eq 'Y' }">
		<div class="section08 max1200">
			...
			<c:if test="${not empty fileList18 and not empty fileList18[0]}">
				<video src="/getImage/<c:out value='${fileList18[0].streFileNm}'/>" width="1200" controls></video>
			</c:if>
		...
		<div class="section13">
			<div class="max1200">
				...
				<c:if test="${not empty fileList32 and not empty fileList32[0]}">
					<video src="/getImage/<c:out value='${fileList32[0].streFileNm}'/>" width="1200" controls></video>
				</c:if>
...
<script>
	$(document).ready(function() {
		//section03 이미지 순서 변경
		$('.section03 > div').eq(0).children('img').insertAfter($('.section03 > div').eq(0).children('div'));

		//1217 추가
		$('.section03 > p').appendTo($('.section03 > div:nth-of-type(1)'));
		$('.section04 > div > p').appendTo($('.section04 > div > div:nth-of-type(2)'));
		$('.section06 > p').appendTo($('.section06 > div:nth-of-type(2)'));
		$('.section10 > p').appendTo($('.section10 > div:nth-of-type(2)'));

		//section03 vs top 1210 > 1217 삭제
		//$('.section03 > p').css('top',$('.section03 h4').outerHeight() + $('.section03 > div').eq(0).outerHeight() + 55 - 30);

		//section04 vs top > 1217 삭제
		//$('.section04 > div > p').css('top',$('.section04 h4').height() + $('.section04 > div > div').eq(0).height() + 55);

		//section06 vs top > 1217 삭제
		//$('.section06 > p').css('top',$('.section06 h4').height() + $('.section06 > div').eq(0).height() + 50);

		//section10 vs top > 1217 삭제
		//$('.section10 > p').css('top',$('.section10 h4').height() + $('.section10 > div').eq(0).height() + 50);
		...
		//section09
		$('.section09 > div > div').wrapAll('<div class="section09_box"/>)');
		$('.section09_box').slick({
			arrows: false,
		  	centerMode: true,
		  	centerPadding: '60px',
