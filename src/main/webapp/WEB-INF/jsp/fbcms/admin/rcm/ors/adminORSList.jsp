...
function fnInit() {
	...
	$("select[name=MEDI_SBJ_CD]").val('${reservationVO.MEDI_SBJ_CD}').prop("selected", true);
...
function fnSelectLinkPage(pageNo) {
	...
	document.reservationInfoForm.action = "<c:url value='/admin/rcm/ors/selectORSList.do'/>";
...
function fnMemberDetail(APPO_NO) {
	...
	document.detailForm.action = "<c:url value='/admin/rcm/ors/selectORSDetail.do'/>";
...
function fnMemberSearch() {
	document.searchForm.action = "<c:url value='/admin/rcm/ors/selectORSList.do'/>";
...
function fnExcelDown() {
	document.searchForm.action = "<c:url value='/admin/rcm/ors/selectORSListExcel.do'/>";
	...
	<table>
		...
		<tbody>
			<tr>
				...
				<th scope="row" class="ta_l">진료과</th>
				<td class="ta_l">
			...
			<tr>
				<th scope="row" class="ta_l">의료진명</th>
				<td class="ta_l">
					<div class="input_wrap">
						<input type="text" class="w_full" name="DR_NM" maxlength="40"
							value="<c:out value='${reservationVO.DR_NM }'/>" onkeypress="enter();">
					</div>
				</td>
	...
	<table>
		...
		<tbody>
			<c:forEach items="${reservationList}" var="reservationInfo"
				...
				<tr>
					<td>
						<c:out value="${reservationListTotCnt - ((reservationVO.pageIndex-1) * reservationVO.pageSize + status.index)}" />
					</td>
					<td class="ta_l">
						<a href="javascript:fnMemberDetail('${reservationInfo.APPO_NO}')"><c:out value='${reservationInfo.MEDI_SBJ_NM}' /></a>
					</td>
