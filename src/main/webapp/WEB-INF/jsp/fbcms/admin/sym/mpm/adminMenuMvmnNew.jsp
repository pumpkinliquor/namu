...
function choiceNodes(nodeNum) {
	...
	parent.document.menuManageVO.upperMenuNo.value = nodeValues[4];
	parent.$('.ui-dialog-content').dialog('close');
}
/* ********************************************************
 * 조회 함수
...
<body>
<form name="searchUpperMenuNoForm" action ="<c:url value='/sym/mnu/mpm/EgovMenuListSelectTmp.do'/>" method="post">
<div style="visibility:hidden;display:none;"><input name="iptSubmit" type="submit" value="전송" title="전송"></div>
<input type="hidden" name="req_RetrunPath" value="/sym/mnu/mpm/EgovMenuMvmn">
<c:forEach var="result" items="${listMenulist}" varStatus="status" >
  ...
  <tr>
    <td>
 		<div class="tree" style="width:400px;">
			...
			if ( typeof document.searchUpperMenuNoForm.req_RetrunPath == "object"
					&& typeof document.searchUpperMenuNoForm.tmp_menuNmVal == "object"
					&& document.searchUpperMenuNoForm.tmp_menuNmVal.length > 0 ) {
				for (var j = 0; j < document.searchUpperMenuNoForm.tmp_menuNmVal.length; j++) {
					Tree[j] = document.searchUpperMenuNoForm.tmp_menuNmVal[j].value;
			    }
