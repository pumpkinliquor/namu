 ...
 ******************************************************** */
 function choiceNodes(nodeNum) {
		...
		document.menuManageVO.upperMenuNo.value = nodeValues[7];
...
function fn_validatorMenuList() {
	...
	if(document.menuManageVO.upperMenuNo.value == ""){alert("<spring:message code="comSymMnuMpm.menuList.validate.upperMenuNo.notNull" />"); return false;} //상위메뉴번호는 Not Null 항목입니다.
	if(!checkNumber(document.menuManageVO.upperMenuNo.value)){alert("<spring:message code="comSymMnuMpm.menuList.validate.upperMenuNo.onlyNumber" />"); return false;} //상위메뉴번호는 숫자만 입력 가능합니다.
...
<table>
  ...
  <tr>
   <td style="vertical-align:top">
	...
	<input type="hidden" name="tmp_menuNmVal" value="${result.menuNo}|${result.upperMenuNo}|${result.menuNm}|${result.progrmFileNm}|${result.menuNo}|${result.menuOrdr}|${result.menuNm}|${result.upperMenuNo}|${result.menuDc}|${result.relateImagePath}|${result.relateImageNm}|${result.progrmFileNm}|">
	</c:forEach>
   ...
   <td style="vertical-align:top">
		...
		<table class="wTable" >
		  ...
		  <tr>
		    <th><spring:message code="comSymMnuMpm.menuList.upperMenuNo" /> <span class="pilsu">*</span></th><!-- 상위메뉴No -->
		    <td class="left">
		    <input name="upperMenuNo" type="text" value=""  maxlength="10" title="<spring:message code="comSymMnuMpm.menuList.upperMenuNo" />" style="width:190px"/>
	        <a id="popupUpperMenuNo" href="/sym/mnu/mpm/EgovMenuListSelectMvmn.do" target="_blank" title="<spring:message code="comSymMnuMpm.menuList.upperMenuNo" />" style="selector-dummy:expression(this.hideFocus=false);"><img src="<c:url value='/images/egovframework/com/cmm/icon/search2.gif' />"
